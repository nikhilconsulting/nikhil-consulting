...
export default function Pricing() {
  ...
  <span className="text-4xl lg:text-5xl font-bold text-white tracking-tight z-10">
    Best Seller Plans
  </span>

  {/* Pricing Cards */}
  <div className="relative z-10 mt-12 grid gap-10 lg:gap-12 max-w-[1400px] w-full mx-auto grid-cols-1 md:grid-cols-2 xl:grid-cols-4 px-4">
    {currentPlans.map((plan) => (
      <div
        key={plan.name}
        className="rounded-3xl p-8 flex flex-col justify-between min-h-[540px] transform transition-transform duration-500 hover:scale-[1.02] origin-center w-full max-w-sm mx-auto bg-[linear-gradient(125deg,_#382933,_#372935,_#372831)] text-gray-100 shadow-md"
      >
        <span className="text-2xl font-semibold mb-2">{plan.name}</span>
        <p className="text-[#8FCDFF] mb-2 text-base font-semibold">Goal : {plan.goal}</p>

        <ul className="space-y-3 mb-2 text-left text-base">
          {plan.features.map((feature) => (
            <li key={feature} className="flex items-start">
              <svg
                xmlns="http://www.w3.org/2000/svg"
                className="w-5 h-5 mr-2 mt-1 flex-shrink-0 text-white"
                fill="none"
                viewBox="0 0 24 24"
                stroke="currentColor"
                strokeWidth={3}
